...
$file_location = Get-Item ".\GS_SS-MatchResolution.ps1"
...
$taskExists = Get-ScheduledTask -TaskName "Match Sunshine or GameStream Resolution" -ErrorAction Ignore
if($taskExists){
...
New-Item -Name "GS_SS-MatchResolution.bat" -Value "powershell.exe -windowstyle hidden -executionpolicy bypass -command `"Start-ScheduledTask -TaskName 'Match Sunshine or GameStream Resolution' | Out-Null`"" -Path "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup" -Force | Out-Null
...
elseif($install -eq $false -or $install -eq "false") {
    ...
    Get-ScheduledTask -TaskName "Match Sunshine or GameStream Resolution" | Stop-ScheduledTask
    Get-ScheduledTask -TaskName "Match Sunshine or GameStream Resolution" | Unregister-ScheduledTask -Confirm:$false
    Remove-Item -Path "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\GS_SS-MatchResolution.bat" -Force -Confirm:$false | Out-Null
